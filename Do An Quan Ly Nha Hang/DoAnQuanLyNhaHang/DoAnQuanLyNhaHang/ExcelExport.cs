...
namespace DoAnQuanLyNhaHang
{
    public class ExcelExport
    {
        ...
        public void ThanhPhanB1(ref Dictionary<string, string> replacer, double pTongTien,string maban,string sohd,string tenkhachhang,string diachi,string sdt,string nguoilaphoadon,string sotienbangchu)
        {
            if (replacer != null)
            {

                string NgayThangNam = "" + DateTime.Now.Day.ToString() + "/" + DateTime.Now.Month.ToString() + "/" + DateTime.Now.Year.ToString();
                replacer.Add("%SoHD", sohd);
                replacer.Add("%SoBan",maban);
                replacer.Add("%TenKhachHang",tenkhachhang);
                replacer.Add("%DiaChi",diachi);
                replacer.Add("%SDT",sdt);
                replacer.Add("%NguoiLapHoaDon",nguoilaphoadon);
                replacer.Add("%SoTienBangChu",sotienbangchu);
                replacer.Add("%NgayThangNam",NgayThangNam);
                replacer.Add("%TongTien", pTongTien.ToString());

            }
        }
        
        public bool ExportKhoa(List<ChiTietHoaDon> dataSource,HoaDon hd, ref string fileName, bool isPrintPreview)
        {
            // Check if data is null
            ...
            string sohd = hd.MaHoaDon;
            NhanVien nv = db.NhanViens.Where(a=>a.MaNhanVien == hd.MaNhanVien).FirstOrDefault();
            string nguoilaphoadon = nv.TenNhanVien;

            string sotienbangchu = dstc.So_chu(double.Parse(hd.ThanhTien.ToString()));
            string tenkhachhang = "Khách lẻ";
            string diachi = "";
            string sdt = "";
            double pTongTien = double.Parse(hd.ThanhTien.ToString());
            // Create replacer
            Dictionary<string, string> replacer = new Dictionary<string, string>();

            ThanhPhanB1(ref replacer,pTongTien,maban,sohd,tenkhachhang,diachi,sdt,nguoilaphoadon,sotienbangchu);
